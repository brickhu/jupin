...
【reason】给用户看的**一句话**，**不超过 60 字**（理想 40–55）。
  ⚠️⚠️ 它的职责是**挑动用户想试一把**，**不是**解释「难在哪」——
     难点已经由词表逐词讲过了（音标 / 句重音 / 每个词的技巧 / 哪里要连读），
     你在这里再复述一遍是**重复，而且更粗**。
  照下面三条判据写（**这是判据，不是模板**，别套成一个固定句式）：
    · **给一个具体的、能立刻验证的动作**：点到某个词或某个音，用户马上能试、
      能自己听出做到没有（"把 invent it 连成一个音"是动作；"要多练"不是）；
    · **"看着简单，其实有坑"** —— 这类句子的全部卖点就在这个反差上；
    · 门槛要**可征服**：是"这里有个卡点"，不是"你水平低"。
  ⚠️⚠️ **绝对不许出现**：
    · **水平标签**（小学 / 初中 / 高中 / 四级 / 六级 / 考研 / GRE / "X 级水平"）——
      级别已经由档位徽章给用户了；再贴一遍是冗余，而且读砸了会变成"连初中水平都不如"。
    · **总结式的泄气话**：「词都很常见」「词都很简单」「句子很短」「不难」——
      实测这几句在 8 条里出现了 5 次，**挑战心态就是从这里泄掉的**（用户 2026-09 指出的）。
      ⚠️ 但**反差铺垫是允许的**：「看着像儿童读物」「慢念谁都行」——
        只要后面立刻接上那个坑。**区别在于：铺垫是过程，盖章是结论。**
  ⚠️ 语气像**一个跟你一起练的人**，不像老师批作业。可以用音标、说人话。
     不贬低用户（"你读不准" → "这个音容易混"）；不空洞（"这句有点难"等于没说）。
  ⭐ **标杆**（用户 2026-09 亲自指的这一条，照它的**质量**写，但别套成同一个句式）：
    「都是 name、reason、justify 套了层壳，别被吓住；真坑是 \`fear itself\` 和结尾的 -rts。」
    拆开看是三层：**先拆掉它的伪装**（"别被吓住"）→ **点出真正卡住人的那一处**（"真坑是…"）
    → 让用户**马上知道该试什么**。这三层缺一层，读起来就不像挑战、像批作业。
  ⚠️⚠️ 两个实测出来的毛病（2026-09 第一批就是这么翻车的），别犯：
    ① **只有动作、没有反差**：「predict 的 -ct 别拆开，invent it 连成一口气念。」
       —— 干巴，像练习册上的建议，用户没有"想试一把"的冲动。先把"看着简单"这个伪装拆开。
    ② **太长**：超过 40 字就没人读完（实测有一条写成了 69 字）。
  ⚠️ 长度：**60 字以内**（理想 40–55）—— 上面那条标杆自己就是 50 字上下。
  ⚠️⚠️ **只点一个卡点**（最多再带一个音）：开始写第二个卡点，就是超长的起点
     （实测那条 65 字的就是一口气点了 counts / failure / continue 三处）。
