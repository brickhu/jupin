...
【reason】给用户看的**一句话**，**不超过 90 字**（卡片上三行小字）。
  ⚠️⚠️ 它的职责是**挑动用户想试一把**，**不是**解释「难在哪」——
     难点已经由词表逐词讲过了（音标 / 句重音 / 每词的技巧 / 哪里要连读），
     在 reason 里再写一遍完整音标就是重复，而且必然比词表粗。

  ⭐ **固定三拍**（顺序不变，但**句式别套死**）：
    ① **定位**：给这句话一个主观判断，制造预期差（"看着像大白话" / "这句真有点硬"）；
    ② **难点**：点 2–3 个难词，**只写词名 + 一句极短提示**（≤8 字，如"重音在 -bez-"）；
       ⚠️ **不要写完整音标** —— 那是词表的活，写进来既重复又把长度顶爆；
    ③ **收益**：⚠️⚠️ **必须落在「口语」上**：跨过这个坑，他**张口说话**会有什么变化。
       写阅读、听力、考试、词汇量的好处都算跑偏（那是另一个产品）。
  ...
  ⚠️⚠️ **绝对不许出现**：
    ...
    · **总结式的泄气话**：「词都很常见」「词都很简单」「句子很短」——
      挑战心态就是从这里泄掉的（用户 2026-09 指出的）。
      ⚠️ 但**反差铺垫是允许的**（"看着像儿童读物""慢念谁都行"）——
        只要后面立刻接上那个坑。**区别：铺垫是过程，盖章是结论。**
    · **不可验证的承诺**：「直逼母语者水平」「口语暴涨」「秒变地道」「彻底掌握」——
      ⚠️ 用户读完**立刻会看到自己的分数**，这种话当场被打脸，比不承诺更伤信任。
      收益要**就近、可验证**："说出来不打结""下次开口不用在脑子里拼""这口节奏你能带走"。

  ⚠️ 语气像**一个跟你一起练的人**，不像老师批作业。可以用音标、说人话，
     不贬低用户（"你读不准" → "这个音容易混"）；不空洞（"这句有点难"等于没说）。

  ⭐ **标杆**（用户 2026-09 亲自认定的那条，照它的**质量**写，别套句式）：
...
⚠️ **不要写 scores 的解释、不要写其他字段**（多写的会被丢掉）：
{
  "articles": [
    { "index": 1, "text": "纠错后的英文", "translation": "自然口语化的中文（别用直译腔）",
      ...
      "reason": "看着像大白话，真正的坎是 fear itself 和结尾的 -rts；这三处顺了，一开口就是那个味儿。",
