 ...
 *
 *   score = (5×① + 3×② + 2×③) / 10   （结果落在 1–5）
 *   ⚠️ 档位 = **把 score 四舍五入到整数**，再 1–2 → 0 初级 │ 3 → 1 中级 │ 4 → 2 高级 │ 5 → 3 专家
 *      ⇒ 切分点在 **2.5 / 3.5 / 4.5**（不是 2/3/4）
 *      ⚠️ 为什么必须是四舍五入（用户 2026-09 纠正）：按 [3,4) 算高级时，
 *         「She sells seashells…」（词汇 2 = 高中）凭发音 5 就能得到 3.2 → 高级 ——
 *         而用户定的是「高中词汇到不了高级」。四舍五入把 3.2 放回中级，
 *         只有真正的 4.x 才是高级。这也是他原始口径「3:中级，4:高级，5:专家」的字面读法。
 *
 * ⚠️⚠️ **三个分要记进正文 JSON（`scores`）**：不是为了给谁看，
 *    是为了让 difficulty **可以被代码验算** —— 档位对不上就说明有一次算错了，
 *    而「静默算错的档位」是这类功能最难发现的一种坏法（见 difficultyFromScores）。
 *
 * ⚠️ 为什么权重是 5/3/2：用户 2026-09 定的（最初给的是 5/4/1，后来调成 5/3/2 ——
 *    发音从 4 降到 3、长度从 1 提到 2，让**词汇真正主导档位**：
 *    否则「六级词汇 + 短句 + 中等发音」会被长度 1 拖在中级，而用户要的是词汇说话）。
 *    ⚠️ 改权重 = 全库档位都可能变 ⇒ 必须整库重跑（pnpm content:regrade --apply）。
 *
 ...
 */
export const DIFFICULTY_WEIGHTS: readonly [number, number, number] = [5, 3, 2]

/**
 * ⭐ 加权分 → 档位的切分点：`<2.5` 初级、`[2.5,3.5)` 中级、`[3.5,4.5)` 高级、`[4.5,5]` 专家。
 * ⚠️ 就是「把 score 四舍五入到整数」：1–2 → 初级、3 → 中级、4 → 高级、5 → 专家。
 * ⚠️ 只写**下界**（2.5/3.5/4.5）：0 档没有下界，正好是「比第一个还小」。
 * ⚠️ 管理台的「改分数 → 实时看档位」也读它（服务端随 bootstrap 下发）——
 *    前端不许自己再写一份阈值，否则调了公式页面就开始说谎。
 */
export const DIFFICULTY_BANDS: readonly [number, number, number] = [2.5, 3.5, 4.5]
 ...
 * ⚠️ 边界是**左闭右开**：[2.5,3.5) 中级、[3.5,4.5) 高级、[4.5,5] 专家；
 *    等价于「score 四舍五入到 1–5，再 1–2 初级 / 3 中级 / 4 高级 / 5 专家」。
 */
...
/**
 * 三个判据分 → 加权总分（1.0–5.0）—— **只给管理台与日志看**，不进正文 JSON。
