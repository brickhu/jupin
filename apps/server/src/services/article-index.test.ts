...
/**
 * ⚠️ 档位 / 标签落库是**派生索引**：真相在正文 JSON 里。
 *    这里钉住的是「正文 → 索引」这一步的取值口径，
 *    它错的表现是「按档位筛出来一条，点进去根本不是那一档」。
 *
 * ⚠️ 难度**只有一个档位**（2026-09 用户纠正）：词汇 / 发音 / 长度是**三个判据**，
 *    由 LLM 合成一个值 —— 这里只搬那**一个值**，没有「第二条轴」可搬。
 */
