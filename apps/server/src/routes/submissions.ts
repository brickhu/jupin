...
import { canSubmitFree } from '../services/cooldown'
...
async function describe(
  ...
  const [rankInfo, leaderboard, previous, meRows] = await Promise.all([
    getRank(articleId, userId),
    getLeaderboardAround(articleId, userId),
    getBestExcluding(articleId, userId, submissionId),
    db.select({ nextFreeAt: users.nextFreeAt }).from(users).where(eq(users.id, userId)).limit(1),
  ])
  ...
  const result: SubmitResponse = {
    ...
    nextFreeAt: (meRows[0]?.nextFreeAt ?? new Date(0)).toISOString(),
    leaderboard,
