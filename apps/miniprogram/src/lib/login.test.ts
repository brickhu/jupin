 ...
 *
 *    「点登录」要区分**两件事** —— 账号已经在服务端（换设备 / 清了缓存）
 *    和账号还不存在。前者必须**直接进去**，后者才谈得上弹授权层。
...
describe('ensureLogin', () => {
  it('⭐ 本地已经有昵称 → 直接放行，一次请求都不发', async () => {
    ...
    await expect(login.ensureLogin()).resolves.toBe(true)
    expect(fetchMe).not.toHaveBeenCalled()
    expect(store.getState().loginSheet).toBe(false)
  })
  ...
  it('⭐ 服务端也不认识我 → 放行=false 且弹出授权层', async () => {
    ...
    await expect(login.ensureLogin()).resolves.toBe(false)
    expect(store.isLoggedIn()).toBe(false)
    expect(store.getState().loginSheet).toBe(true)
  })
