...
> 业务与需求见 [prd.md](../prd.md) · 技术决策见 [spec.md](../spec.md) ·
> **任务与计划见 [plan.md](../plan.md)** · **现状见代码及其注释**。
> 本目录只放**调研、评估、实验、设计方案与历史归档**。

⚠️ **本目录里的文档都不承担「现状」和「任务状态」**：
它们记的是**当时的调研结论、方案、与走过的弯路**。
- 「现在到底什么行为」→ 代码注释；
- 「做没做、做到哪了」→ [plan.md](../plan.md)（唯一任务来源）。
设计稿里的「待定 / 待确认 / 清单」是**写作当时的快照**，不一致时以 plan.md 为准。
...
| 文件 | 内容 | 状态 |
|---|---|---|
| [growth-and-energy.md](design/growth-and-energy.md) | **用户成长体系**（自我超越 / 坚持不懈 / 人中翘楚）+ **能量值**（替换等级徽章与每日挑战次数） | ✅ 已实施（§11 的冲突清单已过时，见 plan.md B5） |
| [reward-system.md](design/reward-system.md) | **奖励系统**（解冻卡 / 能量点的发放规则、触发器枚举、一个发放接口、幂等键） | ✅ 已实施 |
| [payment-and-purchase.md](design/payment-and-purchase.md) | **购买与支付**（能量包商品 / 小程序虚拟支付 / 发货推送 / 对账退款 / pages/me/energy） | ⏳ 待确认（待拍板项见 plan.md A2–A5） |
...
4. **可复现** —— 实验要给出脚本和步骤
5. ⭐ **不复述代码** —— 字段、常量、阈值、表结构指向代码；
   **任务状态一律不写在这里**，写进 [plan.md](../plan.md)。
   同一事实出现在第二个地方就是 bug 的种子（**重复即错误**）。
