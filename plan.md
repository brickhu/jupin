...
- [ ] **B14** 校准难度提示词的**档位分布** —— 实测（B12 之后重判 8 句）：**7 句上移**、5 句挤在「高级」，出现**天花板效应**。根因：清单写成「**出现即加难度**」，而 `the` 的 /ð/、连读几乎每句都有 → 要改成按**密度与叠加**升档，并写明「`the` 的 /ð/ 太普遍，**不单独**构成升档理由」—— 做完的标志：重判后档位至少覆盖 3 档、不挤在单档
