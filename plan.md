...
> ⚠️ **下面这一段是迁移期的存量记录**（2026-09-24 那次大扫除）：当时还没有 ID 约定，
> 已由 **B7** 一次性提交（`git log --grep 'plan B7'`）。
> 此后新完成的条目**不在这里手写** —— 由 `pnpm plan:status` 从 git 派生。
> 只有天生没有 commit 的完成项才写在这里，并标 `[无 commit]`。

⭐ **ID 归档**：`B7`
> 只列 ID，不写状态与日期（那些从 git 派生）。作用有两个：
> ① 提交信息里的 ID 能被 `plan:status --strict` 校验（防手滑打错）；
> ② 已用过的 ID 不复用。
