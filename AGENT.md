...
读完一句就算一天**连续**（Streak），断了会用冻结卡自动补上。

> ⚠️ 连续记的是**你实际提交的那一刻**所在的自然日（北京时间切天，见
> packages/shared/src/day.ts），**与你读的是哪一天排的那一句无关**：从历史卡片
> 点进往日的句子，照样算**今天**；同一句今天读十遍也只算一次。
> （「排期日」只决定成绩挂到哪一天的榜单，两件事刻意分开 ——
> 否则要么允许回到过去补签，要么把今天的读记成上周。
> 实现见 apps/server/src/services/scoring.ts 里 recordRead 那一段，
> 单测在 packages/shared/src/streak.test.ts。）
...
| **征服** | 该竞技场拿到 **≥85 分**，永久保留、只增不减（⚠️ 阈值是一回事，"分怎么算"见下） |
| **总分怎么算** | ⭐ **由词级分算**，不用引擎给的那个总分：`score = 50×绿词比 + 0.5×词均`（绿词 = 词级分 ≥85，与界面标绿同一个数）。理由是引擎总分是**两层平均**（accuracy 本身已是对整句的平均），个词差异被抹平 —— 实测 σ 7.3 → 14.6。见 `packages/shared/src/scoring.ts` |
| **难度** | 对外只暴露 ⭐ ~ ⭐⭐⭐⭐⭐ |
| **免费额度** | **每天 1 次**挑战（⭐ 与句子无关：换一句也不重置，明天会回来） |
| **付费** | 每天最多 **50 次**（硬上限，不是"无限"——脚本刷分同样要拦） |
...
│   │   └── selftest/            # 真机自检 T1–T6
│   ├── components/              # （尚未创建）词级上色文本 · 榜单 · 额度提示
│   ├── workers/
...
录音（RecorderManager，裸 PCM）
  ...
  → 返回 { score, rank, leaderboard, words, dimensions }
```

**服务端**（`routes/submissions.ts`）的顺序不能改，理由都写在文件注释里：
校验路径属于本人 → **幂等检查（必须在门禁之前）** → 挑战门禁（今天还剩几次）→ 分配 seq → 读音频 → 评分 → 落库。

> ⚠️ 幂等必须在门禁之前：反过来的话，用户"重试"会把同一段音频又算成一次挑战 ——
> 而他只是想再发一次。
...
### 测试时被额度卡住 → `pnpm dev:unlock`

```bash
pnpm dev:unlock          # 把本地账号置为会员（每天 1 次 → 50 次）
pnpm dev:unlock:status   # 只看状态，不改
```

**做法是把账号置为会员，而不是改额度判断** —— 会员本来就是每天 50 次，
这是产品的真实机制。用一个已存在的产品路径来解锁，好过为了开发方便在生产代码里开一个 if 口子。
