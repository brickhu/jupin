...
| **难度** | 对外只暴露 ⭐ ~ ⭐⭐⭐⭐⭐ |
| **免费额度** | **每句 1 次**挑战（⭐ 按句子算，不按时钟算） |
| **付费** | 每句最多 **20 次**（硬上限，不是"无限"——脚本刷分同样要拦） |
| **挑战间隔** | 两次提交至少间隔 **2 分钟**（防刷机，**所有人一视同仁**，付费也不免除） |
...
校验路径属于本人 → **幂等检查（必须在门禁之前）** → 挑战门禁（每句额度 + 挑战间隔）→ 分配 seq → 读音频 → 评分 → 落库。

> ⚠️ 幂等必须在门禁之前：反过来的话，用户"重试"会被自己上一次提交造成的**间隔**挡住，
...
```bash
pnpm dev:unlock          # 把本地账号置为会员（每句 1 次 → 20 次）
pnpm dev:unlock:status   # 只看状态，不改
```

**做法是把账号置为会员，而不是改额度判断** —— 会员本来就是每句 20 次，
这是产品的真实机制。用一个已存在的产品路径来解锁，好过为了开发方便在生产代码里开一个 if 口子。

⚠️ 但**挑战间隔（2 分钟）**是所有人都要遵守的，会员也不例外 ——
量的是"提交有多快"，不是"还能读几次"。连着调两次接口会拿到
`429 TOO_FREQUENT`，等一会儿就好，这是设计如此。
