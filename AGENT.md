...
你只要说清**要什么**和**做完的标志**；其余（任务 ID、分支、commit 消息、计划勾选）都是 agent 的事。

| 你说 | 我做什么 |
|---|---|
| 「**加任务**：…」 | 只落进 [plan.md](plan.md)（`- [ ]` + ID + 做完的标志），**不马上开工** |
| 「**做 B1**」/「把 B1 做了」 | 移到「在做」→ 编码 + 注释 → 提交带 `plan B1` → `pnpm plan:sync` |
| 「**新需求**：…」 | **先对齐 [prd.md](prd.md)**（名词 / 概念 / 应该是什么），回来跟你确认，再落任务、再动代码 |
| 「**问**：…」/「看看…」 | 只回答 —— **不落任务、不动代码** |
| 「**并行**：A 和 B 一起做」 | 按下面「多任务并行」开 worktree；碰公共面（`shared` / 迁移）的仍然串行 |

**你要决定的只有三件**：① 优先级（先做哪条）② **push（什么时候发布）** ③ 产品判断（plan.md 的 A 组）。

**每次任务的固定回路**（你能看到的）：

1. 我回一句「已记 B10 …」—— 任务落进 plan.md；
2. 我做完给你结论 + 改了哪些文件；
3. 我 `git commit`（带 `plan <ID>`）→ `pnpm plan:sync` 勾选并归档；
4. 你随时 `pnpm plan:status` 看全局（任务 × git 证据 × 有没有推），`git log --grep 'plan B1'` 看细节；
5. 你说「发」我才 `git push`。

> ⚠️ **不要顺手塞「顺便也把…」**：按流程每条任务都要落一行 + 一次提交，
> 「顺手做的」会成为没记账的改动（历史上那 120 个没挂 ID 的 commit 就是这么来的）。
> 要带就单独说一条，我拆成两条任务。
...
① **拆分并纠错** —— 拆段是**代码按空行**做的（`shared/paragraphs.ts`，一段 = 一条，接受超长句），
   LLM 只负责**纠错 + 译文 + 三个判据分（词汇/发音/长度）与合成档位 + 那句「难在哪」**
→ ② 候选列表**勾选 / 就地改**（已存在的条目默认不勾、生成时跳过）→ **生成入库（草稿）**（fish 出标准音与词级区间）
